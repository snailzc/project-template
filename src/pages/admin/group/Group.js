...
import GroupForm from '@/components/Admin/GroupForm/index.js';
import PowerSimpleTable from '@/components/Admin/PowerSimpleTable/index.js';
import UserConnect from '@/components/Admin//UserConnect/index.js';
import styles from './Group.scss';
import { getConfigElement, searlizeMenu } from '@/utils/util';
...
const getFormItemLayout = () => ({   // 自定义formItem样式
  labelCol: { span: 8, offset: 0 },
  wrapperCol: { span: 16, offset: 0 },
});

const getSearchFormLayout = () => ({   // 自定义搜索表单元素样式
  lg: { span: 5, offset: 0 },
});

const getBtnFormLayout = () => ({    // 自定义按键表单元素样式
  lg: { span: 7, offset: 0 },
});

const formItemLayout = getFormItemLayout();
const searchFormLayout = getSearchFormLayout();
const btnFormLayout = getBtnFormLayout();
...
class MenuManagePage extends PureComponent {
  state = {
    page: 1,
    limit: 20,
    expandedKeys: [],
  ...
  componentDidMount() {
    ...
    const resultMenu = searlizeMenu(menus);
  ...
  getRoleByUsername = (e) =>{
    e.preventDefault() ;
    this.props.dispatch({
      type: 'group/fetchTypeByUser',
      payload: {
        id: localStorage.currentKey,
        searchGroup: this.state.searchParent,
      },
      uri:pageElement && pageElement.get_by_username && pageElement.get_by_username.uri
    });
  }
  ...
  renderSimpleForm(typeList) {
    return (
      <Form layout="inline" className={classString}>
        <Row style={{paddingLeft: '50px' }} gutter={{ md: 8, lg: 24, xl: 48 }}>
          {
            pageElement && pageElement.btn_add ? <Button type="dashed" icon="plus" size="small" style={{ marginRight: '10px'}} onClick={this.handleAdd}>{ pageElement.btn_add.name }</Button> : ''
          }
          {
      ...
      >
        <Row style={{ display:`flex`,alignItems:`center` }} >
          <Col {...searchFormLayout}>
            <FormItem {...formItemLayout} label="关键字" style={{minHeight:`50px`,display:`flex`,alignItems:`center`}} >
              <Input
                size="small"
          ...
          <Col {...btnFormLayout}>
            ...
            {
              pageElement && pageElement.get_by_username ?
              <span className={styles.submitButtons}>
                <Button
                  size="small"
                  className="ant-btn-plus-margin"
                  icon="search"
                  type="primary"
                  onClick={this.getRoleByUsername}
                >
                  按用户名查询
                </Button>
              </span>:''
      ...
      <>
        <div className={styles.tableList}>
          ...
          <Row gutter={24} className={styles.rows}>
            <Col span={8} style={{ position: 'absolute', top: 0, left: 0, bottom: '10px', overflowX: 'hidden', overflowY: 'auto' }}>
              {
                typeList.length ? (
                  <Tree
                    defaultExpandAll
                ...
                ) : <p style={{margin:`10px 20px`}} >暂无数据</p>
